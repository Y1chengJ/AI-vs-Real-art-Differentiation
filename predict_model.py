...
def evaluate_model(test_loader, model, device, num_classes):
    all_preds = []
    all_labels = []
    label_correct = [0 for _ in range(num_classes)]
    label_totals = [0 for _ in range(num_classes)]
    ...
    per_class_accuracy = [correct / total if total > 0 else 0 for correct, total in zip(label_correct, label_totals)]
    f1 = f1_score(all_labels, all_preds, average='weighted')
    return overall_accuracy, f1, per_class_accuracy

# Evaluate the model
overall_accuracy, f1, per_class_accuracy = evaluate_model(test_loader, model, device, num_classes)
...
for idx, accuracy in enumerate(per_class_accuracy):
    label = list(dataset.label_to_idx.keys())[list(dataset.label_to_idx.values()).index(idx)]
